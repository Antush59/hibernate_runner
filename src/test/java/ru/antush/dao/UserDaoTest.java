...
import ru.antush.util.HibernateTestUtil;
...
class UserDaoTest {
    ...
    void findLimitedUsersOrderedByBirthday() {
        ...
        List<String> fullNames = results.stream().map(User::fullName).collect(toList());
        assertThat(fullNames).containsExactlyInAnyOrder("Diane Greene", "Steve Jobs", "Bill Gates");
    ...
    void isItPossible() {
        ...
        List<String> names = results.stream().map(a -> a.get(0, User.class).fullName()).collect(toList());
        assertThat(names).contains("Sergey Brin", "Steve Jobs");

        List<Double> averagePayments = results.stream().map(a -> a.get(1, Double.class)).collect(toList());
